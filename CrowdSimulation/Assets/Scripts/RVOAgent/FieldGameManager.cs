...
public class FieldGameManager : SingletonBehaviour<FieldGameManager>
{
    ...
    private Dictionary<int, FieldGameAgent> m_agentMap = new Dictionary<int, FieldGameAgent>();
        ...
        for (int i = 0; i < columns; i += 1)
        {
            for (int x = 0; x < files; x += 1)
            {
                ...
                if (sid >= 0)
                {
                    ...
                    FieldGameAgent ga = go.GetComponent<FieldGameAgent>();
                    Assert.IsNotNull(ga);
                    ga.sid = sid;
                    ga.fieldGenerator = this.ffg;
