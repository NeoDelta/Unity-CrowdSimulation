...
namespace Tayx.Graphy.Fps
{
    public class G_FpsMonitor : MonoBehaviour
    {
        ...
        [SerializeField] private    int             m_averageSamples            = 5;
        ...
        private List<float> averageSamples;
        ...
        private void Update()
        {
            ...
            if (averageSamples.Count < m_avgFpsSamplesCapacity)
            {
                averageSamples.Add(m_currentFps);
            }
            else
            {
                averageSamples.RemoveAt(0);
                averageSamples.Add(m_currentFps);
            }

            if (m_timeToResetMinMaxFps > 0)
            {
                for (int i = 0; i < averageSamples.Count; i++)
                {
                    m_avgFps += averageSamples[i];
                }
